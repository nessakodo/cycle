...
if [ ! -f "${SCRIPT_DIR}/.env" ]; then
    ...
    echo "    - KALSHI_PRIVATE_KEY_PATH (path to PEM file on droplet)"
    echo ""
    echo "  Optional (paper testing):"
    echo "    - FINNHUB_API_KEY    (finnhub.io — replaces Glassnode)"
    echo "    - ODDS_API_KEYS      (key1,key2,...,key6 — 6 keys for rotation)"
    echo "    - TRADIER_ACCESS_TOKEN, TRADIER_ACCOUNT_ID (sandbox for paper)"
    echo "    - NEWSAPI_KEY, X_BEARER_TOKEN"
    echo ""
else
...
echo ""
echo "  2. Paper test first:  source venv/bin/activate && python3 main.py"
echo "     Watch for: Finnhub + Odds API (rotating keys) + Tradier sandbox"
